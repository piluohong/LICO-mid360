...
bool IMUInitializer::ActiveInitialIMUState() {
  ...
  if (std::fabs(q_last.x()) > 1e-9) {
    ...
    Eigen::Vector3d ACC_BIAS(2.6630975523772447e-04,1.6972573784404112e-04,7.7439754627203219e-04);
    Eigen::Vector3d GYRO_BIAS(2.2364472813436421e-05,2.2667820610429897e-05,8.7427148367802434e-05);

    imu_state_.bias.accel_bias = ACC_BIAS; //Eigen::Vector3d::Zero();
    imu_state_.bias.gyro_bias = GYRO_BIAS; //Eigen::Vector3d::Zero();
