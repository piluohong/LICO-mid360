...
namespace cocolic
{
      ...
      : fea_param_(LiDARFeatureParam(node["VLP16"]))
  {
    ...
    point_filter_num = node["VLP16"]["point_filter_num"].as<int>();
    min_dist = node["VLP16"]["min_distance"].as<float>();
      ...
      RTPointCloud::Ptr out_cloud)
  {
    ...
    if (!has_checked)
    {
      ...
      if (!has_ring_field)
        LOG(WARNING) << "[ParsePointCloudNoFeature] input cloud NOT has [ring] field";

      if (!has_time_field && !has_t_field && !has_timestamp_field)
        LOG(WARNING)
            << "[ParsePointCloudNoFeatureud] input cloud NOT has [time] or [t] or [timestamp] field";
      if (has_time_field)
        std::cout << "is velodyne \n";
      if (has_t_field)
        std::cout << "is ouster \n";
    ...
    if (check_field_passed)
    {
      if (has_time_field)
      {
        ...
        for (size_t i = 0; i < plsize; i++)
        {
          ...
          added_pt.time = fabs(int64_t(pt.time * 1e9)); // s -> ns
          // if (i == 0)
        ...
        p_corner_cloud->push_back(p_surface_cloud->points[0]);
        std::cout << "PointCloud ready" << std::endl;
      ...
      else if (has_timestamp_field)
      {
        ...
        double first_timestamp = tmp_out_cloud->points[0].timestamp;
        for (int i = 0; i < plsize; i++)
        {
          const auto &pt = tmp_out_cloud->points[i];
          RTPoint added_pt;
          ...
          added_pt.intensity = pt.intensity;
          added_pt.ring = pt.ring;
          added_pt.time = int64_t((pt.timestamp - first_timestamp) * 1e9); // s->ns
          out_cloud->points[i] = added_pt;
          p_full_cloud->points[i] = added_pt;
          if (i % point_filter_num == 0)
          {
    ...
    static int cnt = 0;
    if (cnt % 1 == 0)
       ROS_INFO("Cur points sur cor full : %d, %d ,%d ",p_surface_cloud->size(),p_corner_cloud->size(),p_full_cloud->size());
    cnt++;  
