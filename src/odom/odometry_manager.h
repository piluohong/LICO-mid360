...
public:
    // save global PCD map
    std::vector<PosCloud> all_globalmap;
    std::vector<ColorPointCloud> all_RGBglobalmap;
    inline void saveGlobalmap()
    {
      std::cout << "Saving map ....." << std::endl;;
      PosCloud savecloud;
      ColorPointCloud RGBcloud;
      int frames = 0, pt_cnt; 
      for (int i = 0; i <  all_globalmap.size(); i++)
      {
        if (i % 1 ==0){
          savecloud += all_globalmap[i];
          frames++;
          pt_cnt += savecloud.size();
        }
      }
      std::cout << "Frame's num:" << frames << " , All points"<<" : " << pt_cnt << std::endl;;
      for (int i = 0; i <  all_RGBglobalmap.size(); i++)
      {
        if (i % 1 ==0){
  ...
  inline void savekntsNum(std::string filename)
  {
      std::cout << "Saving kntsVec ....." << std::endl;

      
      std::string dirpath = filename.substr(0, filename.find_last_of('/'));
      mkdir(dirpath.c_str(), 0777); // 创建目录（如果不存在）
      ...
      int sum = 0;
      for (const auto &value : cp_num_vec) {
        sum += value;
      ...
      std::cout << "Sum knts " << sum << "." << std::endl;
  }
  std::vector<double> time_vec;
  inline void saveCosttime(std::string filename)
  {
      std::cout << "Saving time cost ....." << std::endl;
      ...
      std::cout << "Time Data saved to " << filename << " successfully." << std::endl;
      std::cout << "Avg timecost " << sum / time_vec.size() << " ms." << std::endl;
  }
