...
namespace cocolic
{
  ...
  public:
    PosCloud::Ptr cur_frame;
    // inline VPointCloud getcur_frame()
    // {
    //   return cur_frame;
    // }
    ...
    void SetPublisher(ros::NodeHandle &nh)
    {
      ...
      pub_cloud_pgo_ =
          nh.advertise<sensor_msgs::PointCloud2>("/lio/cloud_pgo", 10);
      pub_odom_ = nh.advertise<nav_msgs::Odometry>("/lidarAndcameraOdom", 100);
    ...
    void PublishLidarAndCameraOdom(Eigen::Quaterniond quat, Eigen::Vector3d pos, ros::Time timestamp)
    {
      ...
      odom_ros.pose.pose.position.x = pos[0];
      odom_ros.pose.pose.position.y = pos[1];
      odom_ros.pose.pose.position.z = pos[2];

      odom_ros.pose.pose.orientation.w = quat.w();
      odom_ros.pose.pose.orientation.x = quat.x();
      odom_ros.pose.pose.orientation.y = quat.y();
      odom_ros.pose.pose.orientation.z = quat.z();
    ...
    { // feature_cur_ds_
      ...
      if (pub_source_dense_cloud_.getNumSubscribers() != 0)
      {
        ...
        pcl::toROSMsg(source_cloud, source_msg);
        ros::Time time;
        time.fromNSec((long)source_feature.timestamp);
        source_msg.header.stamp = time;//ros::Time::now();
