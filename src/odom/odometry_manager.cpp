...
namespace cocolic
{
      ...
      : odometry_mode_(LIO), is_initialized_(false)
  {
    ...
    msg_manager_ = std::make_shared<MsgManager>(node, nh);  // load rosbag and livoxhandle & imuhandle & imagehandle func.
    ...
    double t0 = omp_get_wtime();
     /// [2] static initialization, do not move at the begging!
      ...
      if (PrepareMsgs())
      {
        ...
        double t3 = (t1 - t0) * 1000;
        time_vec.push_back(t3);
        // deep copy
  ...
  void OdometryManager::ProcessLICData()
  {
    ...
    // std::cout << msg.lidar_surf_cloud->size() << std::endl;
    ...
    /// [3] update lidar local map -> instead to use ikd-Tree or iVox
    ...
    /// [4] update visual local map（tracking map points for the current image frame 为当前图像帧更新跟踪的地图点）
    ...
    if (process_image)
    {
      ...
      if (odom_viewer_.pub_track_img_.getNumSubscribers() != 0 || odom_viewer_.pub_sub_visual_map_.getNumSubscribers() != 0)
      {
        ...
        {
          RGB_pts *rgb_pt = ((RGB_pts *)it->first);
            cv::circle(img_debug, it->second, 4, cv::Scalar(255, 0, 255), 5, 8);  // optical flow + ransac *2 -> 2d association（green）
            VPoint temp_map;
    ...
    double t0 = omp_get_wtime();
    lidar_handler_->localmap_incremental(latest_feature_before_active_time.timestamp,lidar_handler_->use_ivox_); //1: ivox; 0 : ikdtree
    double t1 = omp_get_wtime();
    if (lidar_handler_->use_ivox_)
    {
      std::cout << "Update ivox : " << (t1 - t0) * 1000 << " ms" << std::endl;
    }else{
    ...
    if (process_image)
    {
      ...
      // {
        ...
        for (int i = 0; i < cloud_undistort->points.size(); i++)
        {
          ...
          cv::Point2f pix(std::round(K_(0, 0) * X / Z + K_(0, 2)), std::round(K_(1, 1) * Y / Z + K_(1, 2)));
          
          if(cloud_distort->points[i].x < 0 || pix.x < 0 || pix.y < 0 || pix.x > img_debug.cols || pix.y > img_debug.rows)
              continue;  
          ...
          all_RGBglobalmap.push_back(color_cloud);
  ...
  void OdometryManager::UpdateOneSeg()
  {
    ...
    /// update the first seg
    {
      ...
      if (non_uniform_)
      {
        cp_add_num = GetKnotDensity(aver_r.norm(), aver_a.norm()); // 根据imu状态获取控制点数量
      }
