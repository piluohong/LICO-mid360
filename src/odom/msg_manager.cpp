...
namespace cocolic
{
        ...
        image_topic_("")
  {
    ...
    MODE mode = MODE(node["mode"].as<int>()); 
      ...
      T_LktoL0_vec_.push_back(T_Lk_to_L0);
          ...
          if (use_livox)
            livox_feature_extraction_ =
                std::make_shared<LivoxFeatureExtraction>(lidar_node); // 實例化livox處理類
          if (use_vlp)
        ...
        std::cout << YELLOW << "\t- online mode\n" << RESET << std::endl;
          ...
          if (if_compressed_){
            sub_image_ = nh.subscribe<sensor_msgs::CompressedImage>(
            image_topic_, 2000, &MsgManager::ImageMsgHandle, this,ros::TransportHints().tcpNoDelay());
            }else{
        ...
        for (size_t i = 0; i < lidar_topics_.size(); ++i) {
              ...
              subs_vlp16_.push_back(nh.subscribe<sensor_msgs::PointCloud2>(
                  lidar_topics_[i], 0,
                  boost::bind(&MsgManager::VelodyneMsgHandleNoFeature, this, _1, i)));
              // subs_vlp16_.push_back(nh.subscribe<sensor_msgs::PointCloud2>(
              //   lidar_topics_[i], 0,
              //   boost::bind(&MsgManager::VelodyneMsgHandle, this, _1, i)));
            } else {
              subs_livox_.push_back(nh.subscribe<livox_ros_driver2::CustomMsg>(
                  lidar_topics_[i], 0,
  ...
  void MsgManager::SpinBagOnce()
  {
    ...
    // rosbag::MessageInstance 单条ROS消息的类
  ...
  {
    RTPointCloud::Ptr vlp_raw_cloud(new RTPointCloud);
    timestamps_buf_.push_back(vlp16_msg->header.stamp);
  ...
  {
    // odom_timestamp = livox_msg->header.stamp.toNSec();
    timestamps_buf_.push_back(livox_msg->header.stamp);
    ...
    // livox_feature_extraction_->ParsePointCloudR3LIVE(livox_msg, livox_raw_cloud); //AVIA
  ...
  void MsgManager::ImageMsgHandle(const sensor_msgs::ImageConstPtr &msg)
  {
    ...
    nerf_time_.push_back(image_buf_.back().timestamp);
     ...
     if (image_buf_.back().image.cols == 1280)
     {
