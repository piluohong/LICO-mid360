...
def listener():
  ...
  rospy.Subscriber("/time_update_localmap", Float32, exploredVolumeCallback)
  rospy.Subscriber("/time_find_corresponds", Float32, travelingDistanceCallback)

  fig=plt.figure(figsize=(8,7))
  fig1=fig.add_subplot(311)
  plt.title("Exploration Metrics\n", fontsize=14)
  plt.margins(x=0.001)
  fig1.set_ylabel("Time_update_localmap (ms)", fontsize=12)
  l1, = fig1.plot(time_list2, explored_volume_list, color='r', label='Update localmap')
  fig2=fig.add_subplot(312)
  fig2.set_ylabel("Time_find_corresponds (ms)", fontsize=12)
  l2, = fig2.plot(time_list3, traveling_distance_list, color='r', label='Find corresponds')
